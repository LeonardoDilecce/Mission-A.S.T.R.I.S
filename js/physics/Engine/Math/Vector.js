class Vector 
{
    ...
    clone() {
        return new Vector(this.x, this.y, this.origine);
    }
    add(v) {
        this.x += v.x;
        this.y += v.y;
    ...
    scale(fattore) {
        return new Vector(this.x * fattore, this.y * fattore, { ...this.origine });
    }
    multiply(scalar) {
        this.x *= scalar;
        this.y *= scalar;
    ...
    static fromPoints(p1, p2) {
        return new Vector(p2.x - p1.x, p2.y - p1.y, p1);
    }
    static fromAngle(angle, modulo = 1, origine = { x: 0, y: 0 }) {
        return new Vector(Math.cos(angle) * modulo, Math.sin(angle) * modulo, origine);
    }
}